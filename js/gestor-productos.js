...
async function intentarLogin() {
    ...
    try {
        ...
        // Verificar Permisos
        const cargosPermitidos = ['Gestor de Productos', 'Administrador'];
        if (!cargosPermitidos.includes(empleado.cargo)) {
            errorEl.textContent = 'No tienes permisos para acceder a esta sección';
            errorEl.style.display = 'block';
            return;
        }
...
function agregarFilaColor(color = '', url = '') {
    ...
    div.innerHTML = `
        <input type="text" class="form-control color-name" value="${color}" placeholder="Color" style="flex:1;">
        <input type="text" class="form-control color-url" value="${url}" placeholder="URL Imagen" style="flex:2;">
        <button type="button" class="btn btn-sm" onclick="this.parentElement.remove()">🗑️</button>
    `;
...
function agregarFilaTalla(talla = '', color = '', a = 0, l = 0, j = 0) {
    ...
    tr.innerHTML = `
        <td><input type="text" class="form-control form-control-sm talla-color" value="${color}"></td>
        <td><input type="text" class="form-control form-control-sm talla-name" value="${talla}"></td>
...
async function guardarProducto() {
    ...
    try {
        ...
        await guardarInventarios(res.data.id);
